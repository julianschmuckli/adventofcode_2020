...
LOOP AT lv_input INTO wa_input.
  IF lines( lv_input ) GE ( sy-tabix + 25 ).
    ...
    IF lv_checked EQ abap_false.
      WRITE: 'Task 1 - Found value: ' && wa_cur_result-row.
      EXIT.
    ENDIF.
  ELSE.
    EXIT.
...
" Task 2
LOOP AT lv_input INTO DATA(wa_input_a).
  DATA(lv_cur_range) = sy-tabix.
  LOOP AT lv_input INTO DATA(wa_input_b).
    DATA(lv_cur_range_sum) = 0.
    DATA(lv_cur_range_index) = sy-tabix.
    DO lv_cur_range TIMES.
      READ TABLE lv_input INTO DATA(lv_cur_num) INDEX ( sy-index + lv_cur_range_index - 1 ).
      IF sy-index EQ 1.
